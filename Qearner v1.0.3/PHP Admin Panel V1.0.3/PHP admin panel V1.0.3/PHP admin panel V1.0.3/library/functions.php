...
class Functions {
    ...
    public function get_configurations() {
        ...
        $sql = "SELECT * FROM settings WHERE type <> 'terms_conditions' AND type <> 'privacy_policy' AND type <> 'instructions' AND type <> 'about_us' ";
        $this->db->sql($sql);
    ...
    public function get_custom_coin() {
        $this->db->sql("SET NAMES 'utf8'");
        ...
        if (!empty($res)) {
            ...
            $data['amount'] = '0';
            $data['paypal_coin'] ='0';
            $data['paypal_amount'] = '0';
            $data['true_answer'] = '0';
            $data['penalty_wrong_answer'] = '0';
            $data['daily_quiz_coin'] = '0';
            $data['battle_quiz_entry_coin'] = '0';
            $data['battle_winner_coin'] = '0';
            $data['refer_coin'] = '0';
            $data['earn_coin'] = '0';
            $data['reward_coin'] = '0';
            $data['max_app_coin'] = '0';
    ...
    public function credit_coins_to_friends_code($friends_code) {
        $coin_config = $this->get_custom_coin();

        $sql = "UPDATE `users` SET `coins` = `coins` + " . $coin_config['earn_coin'] . " WHERE `refer_code`='" . $friends_code . "'";
        $this->db->sql($sql);
        $res = $this->db->getResult();

        
        $sql1 = "SELECT * FROM users WHERE refer_code='" . $friends_code . "'";
        $this->db->sql($sql1);
        $res1 = $this->db->getResult();
        
       
        $data = array(
            'uid'	 => $res1[0]['firebase_id'],
            'points' => $coin_config['earn_coin'],
            'type' => 'Earn Using Reference Code',
            'coin_status' => '0',
            'user_id' => $res1[0]['id'],
        ...
        $this->db->insert('tbl_tracker',$data);  // Table name, column names and respective values
